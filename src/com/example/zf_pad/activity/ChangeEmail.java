...
public class ChangeEmail extends Activity implements OnClickListener{
	...
	public void onClick(View v) {
		...
		case R.id.tv_msg:
			if (!StringUtil.isNull(login_edit_name.getText().toString().trim())) {
				API.getUpdateEmailDentcode(ChangeEmail.this,MyApplication.NewUser.getId(),login_edit_name.getText().toString().trim(),
	...
	private boolean check () {
		...
		if (phoneOld.length() == 0) {
			MyToast.showToast(getApplicationContext(),"请输入您的原邮箱号");
			return false;
		}
		...
		if (!StringUtil.checkEmail(phone2)) {
			MyToast.showToast(getApplicationContext(),"请输入正确的邮箱号");
			return false;
		}
		return true;
	}
